...
function displayAllStudents() {
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    const namePart = student.name.padEnd(21);
    const idPart = String(student.id).padEnd(13);
    const scoresPart = student.scores.join(', ').padEnd(21);
    const averagePart = average.toFixed(2);

    console.log(`${namePart}${idPart}${scoresPart}${averagePart}`);
  }
}
